...
test('VTimePickerTitle.js', ({ mount }) => {
  ...
  it('should emit event when clicked on am/pm', async () => {
    ...
    wrapper.find('.v-time-picker-title__ampm .picker__title__btn.active')[0].trigger('click')
    expect(period).not.toBeCalled()
    wrapper.find('.v-time-picker-title__ampm .picker__title__btn:not(.active)')[0].trigger('click')
  ...
  it('should emit event when clicked on hours/minutes', async () => {
    ...
    wrapper.find('.v-time-picker-title__time .picker__title__btn')[1].trigger('click')
    expect(selectingHour).not.toBeCalled()
    wrapper.find('.v-time-picker-title__time .picker__title__btn')[0].trigger('click')

export default {
  ...
  alert1: 'Чтобы ваше приложение работало правильно, вы **должны** обернуть его в компонент `v-app`. Этот компонент необходим для определения контрольных точек сетки для макета. Он может существовать везде внутри body, но должен быть родителем компонентов **ВСЕХ** Vuetify.',
  alert2: 'Применяя **app** prop автоматически применяет положение: **fixed** к элементу макета. Если ваше приложение требует элемент _absolute_, вы можете перезаписать эту функцию, используя **absolute** prop.',
  toc: [
    {
      text: 'Разметка',
      href: 'introduction'

export default {
  ...
  stylusText5: 'Имейте в виду, что относительное расположение node_modules может отличаться в вашем проекте, поэтому отрегулируйте его соответствующим образом. Рекомендуется, чтобы импорт был помещен в основное приложение `index.js` или `client-entry.js` файл. **НЕ** импортируйте свой `main.styl` внутри компонента. Это _желание_ вызывает проблемы с производительностью и резко замедляет **HMR** (перезагрузка горячего модуля).',
  ...
  stylusText7: 'Теперь, когда настроен stylus, вы можете установить значения по умолчанию для переменных стилуса, которые вы хотите изменить. Они должны быть объявлены перед импортом и будут автоматически отменять значения по умолчанию Vuetify.',
  toc: [
    {
      text: 'Тема',
      href: 'introduction'

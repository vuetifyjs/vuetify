...
export function useCalendar (props: CalendarProps) {
  ...
  const weekNumbers = computed(() => {
    const firstDayOfYear = props.firstDayOfYear !== undefined ? Number(props.firstDayOfYear) : undefined
    const firstWeekMinSize = firstDayOfYear !== undefined
      ? (firstDayOfYear === 0 ? 7 : firstDayOfYear)
      : undefined

    return weeksInMonth.value.map(week => {
      return week.length ? adapter.getWeek(week[0], props.firstDayOfWeek, firstWeekMinSize) : null
    })
  })

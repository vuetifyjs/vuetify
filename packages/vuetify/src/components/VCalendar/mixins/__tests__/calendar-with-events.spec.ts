...
describe('calendar-with-events.ts', () => {
  ...
  it('should work with event names', async () => {
    ...
    expect(wrapper.vm.eventNameFunction({ start: { date: '2019-02-12', hour: 8, minute: 30, hasTime: true }, input: { Conference: 'Conference' } })).toMatch(/<strong>(|0)8:30(|.AM)<\/strong> Conference/)
  })
  ...
  it('should format time', async () => {
    ...
    expect(wrapper.vm.formatTime(testData1, true)).toMatch(/8:30(.(AM)|$)/)
    expect(wrapper.vm.formatTime(testData2, true)).toMatch(/(5:45(.(PM))|17:45$)/)
    expect(wrapper.vm.formatTime(testData3, true)).toMatch(/9:05(.(PM)|$)/)
    expect(wrapper.vm.formatTime(testData4, true)).toMatch(/(3(.(PM))|15$)/)
  })

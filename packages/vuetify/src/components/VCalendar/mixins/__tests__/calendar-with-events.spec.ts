...
describe('calendar-with-events.ts', () => {
  ...
  it('should format time', async () => {
    ...
    expect(wrapper.vm.formatTime(testData1, true)).toMatch(/^0?8(:30( AM)?|時30分)$/) // 8:30 AM || 08:30 AM || 8:30 || 08:30
    expect(wrapper.vm.formatTime(testData2, true)).toMatch(/^(0?5:45 PM|17:45|17時45分)$/) // 5:45 PM || 05:45 PM || 17:45
    expect(wrapper.vm.formatTime(testData3, true)).toMatch(/^0?9(:05( AM)?|時0?5分)$/) // 9:05 AM || 09:05 AM || 9:05 || 09:45
    expect(wrapper.vm.formatTime(testData4, true)).toMatch(/^(0?3 PM|15|15時)$/) // 3 AM || 03 AM || 15
  })

...
describe.each([
  ...
  it('should return correct isOpen state in prepend slot', async () => {
    ...
    expect(itemsPrepend[0]).toMatchTextContent(/^true$/)
    expect(itemsPrepend[1]).toMatchTextContent(/^false$/)

    await userEvent.click(screen.getByText(/Core team/))
    await waitIdle()
    expect(itemsPrepend[0]).toMatchTextContent(/^true$/)
    expect(itemsPrepend[1]).toMatchTextContent(/^true$/)
    ...
    await userEvent.click(screen.getByText(/Vuetify Human Resources/))
    await waitIdle()
    expect(itemsPrepend[0]).toMatchTextContent(/^false$/)
  })
})

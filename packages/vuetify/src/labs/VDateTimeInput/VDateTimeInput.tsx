...
import { createSegmentedEdit, dateTimeSegments, remainingHint } from '@/composables/segmentedMask'
import { makeTimeFormatProps, maskWithPeriod, useTimeFormat } from '@/composables/timeFormat'
...
export const VDateTimeInput = genericComponent<VDateTimeInputSlots>()({
  ...
  setup (props, { emit, slots }) {
    ...
    const { parseDate, formatDate, parserFormat, fixYear, order, separator } = useDateFormat(props, adapterLocale, isRtl)
    ...
    const formatTemplate = computed(() => `${parserFormat.value} ${timeFormat.formatTemplate.value}`)
    const placeholder = computed(() => props.placeholder ?? formatTemplate.value)
    ...
    function maskDateTime (input: string, caret = -1) {
      return maskWithPeriod(segments.value, input, /[^\d/.\- :]/g, props.format === 'ampm', caret)
    }

    function getHint (input: string) {
      const masked = maskDateTime(input)
      // the period sits outside the sections, so the mask does not count it towards the width
      const done = props.format === 'ampm' && /\s[AP]M$/.test(masked.value)

      return remainingHint(masked.value, done ? '' : formatTemplate.value.slice(masked.width), input)
    }
    ...
    const {
      ...
      text,
    } = createSegmentedEdit(maskDateTime, separator)
            ...
            default: () => (
              <>
                { isFocused.value && !isReadonly.value && (
                  <div class="v-date-time-input__format-hint" aria-hidden="true">

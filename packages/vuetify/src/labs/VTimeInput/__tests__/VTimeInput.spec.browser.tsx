...
describe('VTimeInput', () => {
  ...
  it('should trail the unfilled format behind what is typed', async () => {
    const { element } = render(() => <VTimeInput modelValue={ null } format="ampm" />)

    const input = screen.getByCSS('.v-time-input input')
    expect(input).toHaveAttribute('placeholder', '--:-- --')

    await userEvent.click(element)
    const hint = screen.getByCSS('.v-time-input__format-hint')
    ...
    expect(hint).toHaveTextContent('01:30 --')

    await userEvent.type(input, 'p')
    expect(hint).toHaveTextContent('01:30 PM')
  })

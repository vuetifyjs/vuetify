...
export const makeVTimeInputProps = propsFactory({
  ...
  // format comes from makeTimeFormatProps, VTimePicker's copy defaults to ampm
...
export const VTimeInput = genericComponent<VTimeInputSlots>()({
  ...
  setup (props, { emit, slots }) {
    ...
    const {
      ...
      text,
    } = createSegmentedEdit(timeFormat.maskTime, ':')
    ...
    const placeholder = computed(() => props.placeholder ?? timeFormat.formatTemplate.value)
    const formatHint = computed(() => timeFormat.getHint(text.value))
            ...
            default: () => (
              <>
                { isFocused.value && !isReadonly.value && (
                  <div class="v-time-input__format-hint" aria-hidden="true">

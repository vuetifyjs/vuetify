...
describe('VMonthPicker', () => {
  it('should select a month and emit the correct model value', async () => {
    ...
    const monthButtons = screen.getByCSS('.v-date-picker-months__content')
    ...
    const newMonthButtons = screen.getByCSS('.v-date-picker-months__content')
      .querySelectorAll('.v-btn')
  ...
  it('should toggle to year view and back when selecting a year', async () => {
    ...
    expect(screen.getByCSS('.v-date-picker-months')).toBeTruthy()
    ...
    await wait(200) // Wait for viewMode transition back to months

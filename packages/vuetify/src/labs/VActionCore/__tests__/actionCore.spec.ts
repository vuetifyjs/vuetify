...
import { destroyActionCoreInstance, useActionCore } from '../actionCore'
import type { ActionContext, ActionCorePublicAPI, ActionDefinition } from '../types'
...
describe('ActionCore', () => {
  ...
  describe('Action Source Management', () => {
    ...
    it('should register a function action source and call it', async () => {
      ...
      expect(core.allActions.value).toHaveLength(1) // This will trigger the computed
  ...
  describe('Hotkey Registration and Triggering', () => {
    it('should register hotkeys with keyBindings.on when actions with hotkeys are processed', async () => {
      ...
      expect(resolvedActions).toHaveLength(7) // Updated expected length
      await nextTick()
...
describe('ActionCore Advanced Capabilities', () => {
  ...
  it('isComponentIntegrationEnabled should work based on ActionCoreOptions', () => {
    ...
    const core2 = useActionCore({ componentIntegration: true })
    ...
    const core3 = useActionCore({ componentIntegration: { VBtn: true, VChip: false } })
    ...
    const core4 = useActionCore({ componentIntegration: false })

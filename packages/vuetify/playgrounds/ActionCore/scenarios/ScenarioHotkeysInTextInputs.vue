<template>
  <scenario-card title="Hotkeys In Text Inputs">
    <v-text-field
      ...
      label="Special Input (press Enter)"
      @focus="inputFocused = true"
      @blur="inputFocused = false"
      class="mb-2"
    />
    <p>Press <strong>Enter</strong> while focused to trigger the action.</p>
...
watchEffect(() => {
  if (inputFocused.value && logAction) {
    logAction('Special input focused (ScenarioHotkeysInTextInputs)')
  }
})
</script>

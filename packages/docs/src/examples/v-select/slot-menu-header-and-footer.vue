  ...
  >
    ...
    <template v-slot:menu-footer="{ search, filteredItems }">
      <div class="d-flex align-center pa-2 pl-4 border-t">
        <span v-if="search.value" class="text-body-2">Found {{ filteredItems.length }} of {{ items.length }}</span>
        <v-btn class="ml-auto" text="Done" variant="tonal" @click="menu = false"></v-btn>
      </div>
    </template>
  </v-select>
</template>

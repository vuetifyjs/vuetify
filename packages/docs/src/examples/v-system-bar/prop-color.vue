<template>
  <div>
    <v-layout style="height: 50px">
      <v-system-bar color="primary">
        <v-icon class="ms-2" icon="i-mdi:wifi-strength-4"></v-icon>

        <v-icon class="ms-2" icon="i-mdi:signal-cellular-outline"></v-icon>

        <v-icon class="ms-2" icon="i-mdi:battery"></v-icon>

        <span class="ms-2">08:30</span>
      </v-system-bar>
    </v-layout>

    <v-layout style="height: 50px">
      <v-system-bar color="red-lighten-2">
        <v-icon class="ms-2" icon="i-mdi:wifi-strength-2"></v-icon>

        <v-icon class="ms-2" icon="i-mdi:signal-cellular-outline"></v-icon>

        <v-icon class="ms-2" icon="i-mdi:battery"></v-icon>

        <span class="ms-2">18:30</span>
      </v-system-bar>
    </v-layout>

    <v-layout style="height: 50px">
      <v-system-bar color="indigo-darken-2">
        <v-icon class="ms-2" icon="i-mdi:wifi-strength-3"></v-icon>

        <v-icon class="ms-2" icon="i-mdi:signal-cellular-outline"></v-icon>

        <v-icon class="ms-2" icon="i-mdi:battery"></v-icon>

        <span class="ms-2">13:24</span>
      </v-system-bar>
    </v-layout>
  </div>
</template>

<template>
  <div>
    <v-video
      :start-at="10"
      class="mx-auto"
      controls-variant="mini"
      height="300"
      image="https://jsek.work/vt-sunflowers.jpg"
      rounded="lg"
      src="https://jsek.work/vt-sunflowers.mp4"
      width="500"
      eager
      hide-overlay
      pills
    >
      <template v-slot:controls="{ play, pause, isPlaying, progress, skipTo, volume, toggleMuted, toggleFullscreen }">
        <v-defaults-provider :defaults="{ VIconBtn: { color: 'red', rounded: 'lg', size: '36', variant: 'flat' }, VSlider: { color: 'red', trackColor: 'white' } }">
          <div class="d-flex ga-3 w-100 px-2">
            <v-icon-btn
              :icon="isPlaying ? 'mdi-pause' : 'mdi-play'"
              @click="() => isPlaying ? pause() : play()"
            ></v-icon-btn>
            <v-slider :model-value="progress" width="75%" no-keyboard @update:model-value="skipTo"></v-slider>
            <v-video-volume
              v-model="volume.value"
              :slider-props="{ maxWidth: 100, width: '25%' }"
              class="ga-3"
              inline
              @click="toggleMuted"
            ></v-video-volume>
            <v-icon-btn icon="mdi-fullscreen" @click="toggleFullscreen"></v-icon-btn>
          </div>
        </v-defaults-provider>
      </template>
    </v-video>
  </div>
</template>

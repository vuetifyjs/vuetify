...
<script setup>
  ...
  const showSelect = computed(() => {
    return isEnabled('header.data-table-select') || isEnabled('item.data-table-select')
  })
  const hideHeaders = computed(() => {
    return !showSelect.value
  })
  const isLoading = computed(() => {
    return isEnabled('progress')
  })
  function isEnabled (slot) {
    return enabled.value === slot
  }
  watch(enabled, slot => {
    if (slot === 'no-data') {
      items = []

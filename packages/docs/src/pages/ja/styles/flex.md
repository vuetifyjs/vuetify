...
meta:
  ...
  description: Flex ヘルパークラスを使用すると、フレックスボックスの親と子を変更できます。
  keywords: flex ヘルパークラス、flex クラス、vuetify flex
related:
  - /styles/display/
  - /components/grids
  - /styles/css-reset/
...
` display `ユーティリティを使用すると、任意の要素をフレックスボックスコンテナに変換して、**直接の子要素**をフレックスアイテムに変換できます。 追加の flex プロパティユーティリティを使用すると、それらの操作をさらにカスタマイズできます。
...
The `justify-content` flex setting can be changed using the flex justify classes. This by default will modify the flexbox items on the **x-axis** but is reversed when using `flex-direction: column`, modifying the **y-axis**. `start` (ブラウザのデフォルト), `end`, `center`, `space-between`, `space-around` から選択します。
...
The `align-self` flex setting can be changed using the flex align-self classes. This by default will modify the flexbox items on the **x-axis** but is reversed when using `flex-direction: column`, modifying the **y-axis**. `start`, `end`, `center`, `baseline`, `auto`, `stretch` (ブラウザのデフォルト) から選択します。
...
<alert type="error">
  ...
  **IE11** は、デフォルト以外の `justify-content` 値のある親を持つフレックスアイテムの自動マージンを適切にサポートしていません。 こちらの [StackOverflow answer](https://stackoverflow.com/a/37535548) を参照してください。
...
The `align-content` flex setting can be changed using the flex align-content classes. This by default will modify the flexbox items on the **x-axis** but is reversed when using `flex-direction: column`, modifying the **y-axis**. `start` (ブラウザのデフォルト),`end`, `center`, `between`, `around`, `stretch` から選択します。

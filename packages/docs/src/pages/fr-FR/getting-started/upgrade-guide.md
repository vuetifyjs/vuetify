...
La version 2 contient des modifications de rupture non rétrocompatibles. Cela inclut les fonctionnalités précédemment dépréciées de la version 1.x.x. Ces changements de rupture sont notés dans la console pour les composants correspondants.

La grille existante est toujours opérationnelle et dispose d'un [plugin eslint ](https://github.com/vuetifyjs/eslint-plugin-vuetify) pour faciliter la migration. This plugin can also be used to help upgrade to the [**new grid**](/components/grids).

### Bootstrap

Vuetify doit maintenant être instancié et transmis à l'instance initiale de Vue. Ceci est similaire à la façon dont **vue-router** et **vuex** sont bootstrapés.

#### Installation du plugin Vue-CLI 3 Vuetify
...
Les composants suivants sont maintenant **lazy** par défaut. Cela signifie qu'ils ne rendront pas leur contenu tant qu'ils ne seront pas explicitement activés. Cela améliore considérablement les performances mais peut ne pas être souhaité selon les besoins de votre application _(i.e. Pour les besoins du référencement)_. Pour revenir au comportement précédent, utilisez le prop **eager**.
...
`vuetify/lib` est maintenant compilé en **es6**. Cela signifie que la prise en charge d'IE nécessite l'utilisation de [`transpileDependencies`](https://cli.vuejs.org/config/#transpiledependencies) ou similaire, ainsi que de `@babel/polyfill`. Les dépendances transpiles sont automatiquement ajoutées lors de l'utilisation de vue-cli-3. Si vous disposez d'une version plus ancienne, vous pouvez simplement ajouter "vuetify" à la liste.

### Thème

Supporte désormais les variantes de thèmes sombres/clairs. La propriété _dark_ a été déplacée dans la propriété du thème. Change dynamiquement lorsque l'on bascule **$vuetify.theme.dark**. Si vous n'utilisez qu'une seule variante, il vous suffit de définir ses couleurs.
...
Afin de désactiver la création de la feuille de style du thème, vous devez utiliser la propriété **disable** de l'objet du thème.
...
La déclaration des icônes et des polices d'icônes est désormais placée sous la propriété **icons**.
...
- Par défaut, on utilise maintenant les icônes [mdi](https://materialdesignicons.com/). For information on how to install please navigate [here](/features/icon-fonts#install-material-design-icons)

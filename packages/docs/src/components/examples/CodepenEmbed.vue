...
let embed = function embed (e, target, r) {
  ...
  var a = ["title", "description", "tags", "html_classes", "head", "stylesheets", "scripts"]
  var o = function(e) {
  ...
  var createElement = function (name, attrs) {
    ...
    for (var n in attrs) Object.prototype.hasOwnProperty.call(attrs, n) && el.setAttribute(n, attrs[n]);
    return el
  }
  var createForm = function(e, t) {
    ...
    for (var i in e.data = function(e) {
      ...
      }(t), e) "prefill" !== i && r.appendChild(createElement("input", {
      type: "hidden",
      name: i,
      value: e[i]
    }));
  ...
  var createIFrame = function (attrs) {
    ...
    return "prefill" in attrs == !1 && (n.loading = "lazy"), attrs["slug-hash"] && (n.id = "cp_embed_" + attrs["slug-hash"].replace("/", "_")), createElement("iframe", n)
  }
  var insertFragment = function (el, fragment) {
    if (el.parentNode) {
      var r = document.createElement("div");
      return r.className = "cp_embed_wrapper", r.appendChild(fragment), el.parentNode.replaceChild(r, el), r
    }
